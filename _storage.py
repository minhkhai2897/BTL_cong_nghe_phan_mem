...
class Ranklist(Score):
    ...
    def read_score(self, f, score: Score): 
        ...
        if line:
            ...
            score.add_new_score1(*score_values) 
    ...
    def read_ranklist(self, path) -> list[Score]:
        ...
        with open(path, "r") as f:
            ...
            if len(self._len_list) == 0:  # Check if the string is empty
                self._len_list = 1
            self._len_list = int(self._len_list)

            for i in range(self._len_list):
                self._scores.append(Score())
                self.read_score(f, self._scores[i])
            return self._scores
