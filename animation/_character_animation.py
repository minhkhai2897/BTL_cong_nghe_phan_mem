...
class CharacterAnimation(Animation):
    ...
    def __remove_effect_end(self):
        ...
        self._effect = [effect for effect in self._effect if not effect.is_end()]
    ...
    def __render_effects(self, screen: pygame.surface.Surface, position: tuple[float, float]):
        ...
        for effect in self._effect:
            if (effect.get_name() == "thunder") or (effect.get_name() == "thunder_yellow"):
                ...
                x = position[0] + (self._current_img.get_width() - effect.get_width()) / 2
                y = position[1] + (self._current_img.get_height() - effect.get_height()) / 2
                effect.render(screen, (x, y))
    ...
    def __render_hp(self, screen: pygame.surface.Surface, position: tuple[float, float], hp: int, max_hp: int):
        ...
        LENGTH = min(16, self._current_img.get_width())
        DISTANCE_y = 3

        if (hp < max_hp):
            start_pos = (position[0] + ((self._current_img.get_width() - LENGTH) / 2), position[1] - DISTANCE_y)
            ...
            z2 = hp % max_hp
            for i in range(z1):
                start_pos = (position[0] + ((self._current_img.get_width() - LENGTH) / 2), position[1] - DISTANCE_y * (i + 1))
                end_pos = (start_pos[0] + LENGTH, start_pos[1])
                pygame.draw.line(screen, (0, 0, 255), start_pos, end_pos, 1)
            if (z2 > 0):
                start_pos = (position[0] + ((self._current_img.get_width() - LENGTH) / 2), position[1] - DISTANCE_y * (z1 + 1))
